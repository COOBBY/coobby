 					...
 					<!--<div class="form-widget">
						...
						<div class="row shadow bg-light border">
							<div class="col-lg-12 p-5">
								<form class="row mb-0" id="fitness-form" action="recipeUpdate" method="post" enctype="multipart/form-data">
										...
										<div id="textcontainer">
											<div class="col-10 form-group">
												<div class="row">
													...
													<div class="col-sm-10">
														<input type="text" name="reTitle" id="fitness-form-name" class="form-control" value="${ recipeData.reTitle }" placeholder="제목을 입력해주세요">
													</div>
												</div>
											</div>
											<div class="col-10 form-group">
												<div class="row">
													...
													<div class="col-sm-10">
														<textarea class="introduce" name="reContent" placeholder="내용을 입력해주세요">${ recipeData.reContent }</textarea>
													</div>
												</div>
											</div>
											<div class="col-10 form-group">
												<div class="row">
													...
													<div class="col-sm-10">
														<input type="text" name="reVideoUrl" id="fitness-form-phone" class="form-control" value="${ recipeData.reVideoUrl }" placeholder="URL을 입력하세요">
													</div>
												</div>
											</div>
										</div>
												...
												<div class="btn-group d-flex" role="group">
													<select class="form-select" name="cateKind" id="event-registration-interests">
														...
														<c:forEach items="${ kind }" var="catekind">
															<option value="${ catekind.kindCode }"<c:if test="${ recipeData.cateKind.kindCode eq catekind.kindCode}">selected</c:if>>${ catekind.kindName }</option>
														</c:forEach>
													</select>
													<select class="form-select" name="cateSitu" id="">
														<option value="">상황별</option>
														<c:forEach items="${ situ }" var="catesitu">
															<option value="${ catesitu.situCode }"<c:if test="${ recipeData.cateSitu.situCode eq catesitu.situCode}">selected</c:if>>${ catesitu.situName }</option>
														</c:forEach>
													</select>
													<select class="form-select" name="cateHow" id="event-registration-interests">
														<option value="">방법별</option>
														<c:forEach items="${ how }" var="catehow">
															<option value="${ catehow.howCode }"<c:if test="${ recipeData.cateHow.howCode eq catehow.howCode}">selected</c:if>>${ catehow.howName }</option>
														</c:forEach>
													</select>
													<select class="form-select" name="cateIngr" id="event-registration-interests">
														<option value="">재료별</option>
														<c:forEach items="${ ingr }" var="cateingr">
															<option value="${ cateingr.ingrCode }"<c:if test="${ recipeData.cateIngr.ingrCode eq cateingr.ingrCode}">selected</c:if>>${ cateingr.ingrName }</option>
														</c:forEach>
													</select>
												</div>
											</div>
									...
									<div class="col-12 form-group">
										<c:forEach items="${ ingrData }" var="ingrData">
											<input type="hidden" name="cookNo" value="${ ingrData[2] }" />
											<div id="ingrinputcontainer" class="row">
												<div id="ingrname" class="col-sm-4">
													<input type="text" name="ingrName" id="fitness-form-age" class="form-control" value="${ ingrData[0] }" placeholder="예) 고추장">
												</div>
												<div id="ingrcount" class="col-sm-4">
													<input type="text" name="ingrCount" id="fitness-form-age" class="form-control" value="${ ingrData[1] }" placeholder="예) 2스푼">
												</div>
											</div>
										</c:forEach>
									...
									<div>
										<ol class="list-preparation">
											<c:forEach var="cook" items="${ seqCook }">
												<c:forEach var="img" items="${ cookImg }">
													<li>
														<div class="recipeinsertcontainer">
															<textarea class="recipeinsert" name="reCook" rows="7" placeholder="내용을 입력해주세요">${ cook }</textarea>
															<img id="preview-image" class="recipeimg" src="/resources/user/recipeimages/${ img.reStoredImage }" alt="" />
															<input type="hidden" value="${ img.reStoredImage }" name="reStoredImage" />
														</div>
														<div class="mb-3">
									...
									<div class="row">
										...
										<div class="col-sm-9">
											<textarea class="introduce" name="reTip" placeholder="내용을 입력해주세요">${ recipeData.reTip }</textarea>
										</div>
									</div>
									<div class="col-12 d-flex justify-content-end align-items-center">
										<button type="button" id="calories-trigger" class="btn btn-secondary">목록보기</button>
										<button type="submit" name="fitness-form-submit" class="btn btn-success ms-2">수정하기</button>
									</div>
								</form>
							</div>
